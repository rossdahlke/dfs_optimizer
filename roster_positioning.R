get_positions = function(team = team){
  ...
  team_positions <- team %>% 
    ...
    tidyr::pivot_longer(cols = c(pg, sg, sf, pf, c, g, f, util)) %>% 
    group_by(id) %>% 
  ...
  team_w_positions <- result %>% 
    ...
    right_join(team) %>% 
    mutate(roster_position = factor(dfs_position, levels = c("PG", "SG", "SF", "PF", "C", "G", "F", "UTIL")), .after = last_name) %>% 
    arrange(roster_position) %>% 
    select(-dfs_position)
  
  return(team_w_positions)
